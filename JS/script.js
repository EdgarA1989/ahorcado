  ...
  t.addEventListener("click", () => {
    ...
    if (!listaOK.includes("_")) {
      ...
      setTimeout(function () {
        if (
          confirm(
            `¡GANASTE! Seguro sos equipo CABRAAAAA GANADOR FELICIDADES fueron ${contador} intentos  ¿Quieres reiniciar el juego?`
          )
        ) {
          location.reload();
        }
    ...
    if (contadorErrores == imagenes.length - 1) {
      ...
      setTimeout(function () {
        if (
          confirm(
            `Has superado los  intentos . ¡Perdiste! EQUIPO MALEFICA PERDEDOOOOOR ¿Quieres reiniciar el juego?`
          )
        ) {
          location.reload();
        }
